...
_ASCII_ALIKE: Dict[str, str] = {
    'A': 'ÀÁÂÃÄÅĀĂĄǍǞǠǺȀȂȦȺΆΑᴀᴬḀẠẢẤẦẨẪẬẮẰẲẴẶἈἉἊἋἌἍἎἏᾈᾉᾊᾋᾌᾍᾎᾏᾸᾹᾺΆᾼⒶⱯＡ𝐀𝐴𝑨𝒜𝓐𝔄𝔸𝕬𝖠𝗔𝘈𝘼𝙰🄐🄰🅐🅰🇦',
    'B': 'ƁƂɃʙΒᴃᴮᴯḂḄḆℬⒷＢ𝐁𝐵𝑩𝓑𝔅𝔹𝕭𝖡𝗕𝘉𝘽𝙱🄑🄱🅑🅱🇧',
    'C': 'ÇĆĈĊČƇȻʗᴄḈℂℭⒸＣ𝐂𝐶𝑪𝒞𝓒𝕮𝖢𝗖𝘊𝘾𝙲🄒🄫🄲🅒🅲🇨',
    'D': 'ÐĎĐƉƊƋΔᴅᴆᴰḊḌḎḐḒⅅⒹＤ𝐃𝐷𝑫𝒟𝓓𝔇𝔻𝕯𝖣𝗗𝘋𝘿𝙳🄓🄳🅓🅳🇩',
    'E': 'ÈÉÊËĒĔĖĘĚƐȄȆȨɆʚΈΉΕΗᴇᴱᴲḔḖḘḚḜẸẺẼẾỀỂỄỆἘἙἚἛἜἝἨἩἪἫἬἭἮἯᾘᾙᾚᾛᾜᾝᾞᾟῈΈῊΉῌℰⒺＥ𝐄𝐸𝑬𝓔𝔈𝔼𝕰𝖤𝗘𝘌𝙀𝙴🄔🄴🅔🅴🇪',
    'F': 'ƑɸḞℱⒻＦ𝐅𝐹𝑭𝓕𝔉𝔽𝕱𝖥𝗙𝘍𝙁𝙵🄕🄵🅕🅵🇫',
    'G': 'ĜĞĠĢƓƔǤǦǴʛˠΓᴳḠⒼＧ𝐆𝐺𝑮𝒢𝓖𝔊𝔾𝕲𝖦𝗚𝘎𝙂𝙶🄖🄶🅖🅶🇬',
    'H': 'ĤĦȞʜᴴḢḤḦḨḪℋℌℍⒽⱧＨ𝐇𝐻𝑯𝓗𝕳𝖧𝗛𝘏𝙃𝙷🄗🄷🅗🅷🇭',
    'I': 'ÌÍÎÏĨĪĬĮİƖƗǏȈȊɪΊΐΙΪᴵḬḮỈỊἸἹἺἻἼἽἾἿῘῙῚΊℐℑⒾＩ𝐈𝐼𝑰𝓘𝕀𝕴𝖨𝗜𝘐𝙄𝙸🄘🄸🅘🅸🇮',
    'J': 'ĴɈᴊᴶⒿＪ𝐉𝐽𝑱𝒥𝓙𝔍𝕁𝕵𝖩𝗝𝘑𝙅𝙹🄙🄹🅙🅹🇯',
    'K': 'ĶƘǨΚᴋᴷḰḲḴⓀⱩＫ𝐊𝐾𝑲𝒦𝓚𝔎𝕂𝕶𝖪𝗞𝘒𝙆𝙺🄚🄺🅚🅺🇰',
    'L': 'ĹĻĽĿŁȽʟΛᴌᴸḶḸḺḼℒⓁⱠⱢＬ𝐋𝐿𝑳𝓛𝔏𝕃𝕷𝖫𝗟𝘓𝙇𝙻🄛🄻🅛🅻🇱',
    'M': 'ΜᴍᴹḾṀṂℳⓂⱮＭ𝐌𝑀𝑴𝓜𝔐𝕄𝕸𝖬𝗠𝘔𝙈𝙼🄜🄼🅜🅼🇲',
    'N': 'ÑŃŅŇƝǸȠΝᴎᴺᴻṄṆṈṊℕⓃＮ𝐍𝑁𝑵𝒩𝓝𝔑𝕹𝖭𝗡𝘕𝙉𝙽🄝🄽🅝🅽🇳',
    'O': 'ÒÓÔÕÖØŌŎŐƆƟƠǑǪǬǾȌȎȪȬȮȰɷΌΏΟΩᴏᴑᴓᴼṌṎṐṒỌỎỐỒỔỖỘỚỜỞỠỢὈὉὊὋὌὍὨὩὪὫὬὭὮὯᾨᾩᾪᾫᾬᾭᾮᾯῸΌῺΏῼⓄＯ𝐎𝑂𝑶𝒪𝓞𝔒𝕆𝕺𝖮𝗢𝘖𝙊𝙾🄞🄾🅞🅾🇴',
    'P': 'ƤΠᴘᴾṔṖℙⓅⱣＰ𝐏𝑃𝑷𝒫𝓟𝔓𝕻𝖯𝗣𝘗𝙋𝙿🄟🄿🅟🅿🇵',
    'Q': 'ϞℚⓆＱ𝐐𝑄𝑸𝒬𝓠𝔔𝕼𝖰𝗤𝘘𝙌𝚀🄠🅀🅠🆀🇶',
    'R': 'ŔŖŘȐȒɌʀʁΡᴙᴚᴿṘṚṜṞῤῥῬℛℜℝⓇⱤＲ𝐑𝑅𝑹𝓡𝕽𝖱𝗥𝘙𝙍𝚁🄡🄬🅁🅡🆁🇷',
    'S': 'ŚŜŞŠȘʃʅʆΣṠṢṤṦṨẛⓈＳ𝐒𝑆𝑺𝒮𝓢𝔖𝕊𝕾𝖲𝗦𝘚𝙎𝚂🄢🅂🅢🆂🇸',
    'T': 'ŢŤŦƬƮȚȾΤᴛᵀṪṬṮṰⓉＴ𝐓𝑇𝑻𝒯𝓣𝔗𝕋𝕿𝖳𝗧𝘛𝙏𝚃🄣🅃🅣🆃🇹',
    'U': 'ÙÚÛÜŨŪŬŮŰŲƯǓǕǗǙǛȔȖɄʊΎΥΫϒϓϔᴜᵁṲṴṶṸṺỤỦỨỪỬỮỰὙὛὝὟῨῩῪΎⓊＵ𝐔𝑈𝑼𝒰𝓤𝔘𝕌𝖀𝖴𝗨𝘜𝙐𝚄🄤🅄🅤🆄🇺',
    'V': 'ƲˬᴠṼṾⓋＶ𝐕𝑉𝑽𝒱𝓥𝔙𝕍𝖁𝖵𝗩𝘝𝙑𝚅🄥🅅🅥🆅🇻',
    'W': 'ŴƜǷɯɰϜᴡᵂẀẂẄẆẈⓌＷ𝐖𝑊𝑾𝒲𝓦𝔚𝕎𝖂𝖶𝗪𝘞𝙒𝚆🄦🅆🅦🆆🇼',
    'X': 'ẊẌⓍＸ𝐗𝑋𝑿𝒳𝓧𝔛𝕏𝖃𝖷𝗫𝘟𝙓𝚇🄧🅇🅧🆇🇽',
    'Y': 'ÝŶŸƱƳȜȲɎɥʏẎỲỴỶỸⓎＹ𝐘𝑌𝒀𝒴𝓨𝔜𝕐𝖄𝖸𝗬𝘠𝙔𝚈🄨🅈🅨🆈🇾',
    'Z': 'ŹŻŽƵȤʒʓΖᴢẐẒẔℤℨⓏⱫＺ𝐙𝑍𝒁𝒵𝓩𝖅𝖹𝗭𝘡𝙕𝚉🄩🅉🅩🆉🇿',
    'a': 'àáâãäåāăąǎǟǡǻȁȃȧɐɑɒάαᵃᵄᵅḁẚạảấầẩẫậắằẳẵặἀἁἂἃἄἅἆἇὰάᾀᾁᾂᾃᾄᾅᾆᾇᾰᾱᾲᾳᾴᾶᾷₐ⒜ⓐⱥａ𝐚𝑎𝒂𝒶𝓪𝔞𝕒𝖆𝖺𝗮𝘢𝙖𝚊',
    'b': 'ƀƃɓβϐᵇᵝᵦᵬᶀḃḅḇ⒝ⓑｂ𝐛𝑏𝒃𝒷𝓫𝔟𝕓𝖇𝖻𝗯𝘣𝙗𝚋',
    'c': 'çćĉċčƈȼɕϲḉ⒞ⓒｃ𝐜𝑐𝒄𝒸𝓬𝔠𝕔𝖈𝖼𝗰𝘤𝙘𝚌',
    'd': 'ðďđƌƍȡɖɗδᵈᵟᵭᶁḋḍḏḑḓⅆ⒟ⓓｄ𝐝𝑑𝒅𝒹𝓭𝔡𝕕𝖉𝖽𝗱𝘥𝙙𝚍🆥',
    'e': 'èéêëēĕėęěȅȇȩɇɘɛɜɝɞέήεηᴈᵉᵋᵌḕḗḙḛḝẹẻẽếềểễệἐἑἒἓἔἕἠἡἢἣἤἥἦἧὲέὴήᾐᾑᾒᾓᾔᾕᾖᾗῂῃῄῆῇₑℯⅇ⒠ⓔｅ𝐞𝑒𝒆𝓮𝔢𝕖𝖊𝖾𝗲𝘦𝙚𝚎',
    'f': 'ƒᵠᵩᵮᶂḟ⒡ⓕｆ𝐟𝑓𝒇𝒻𝓯𝔣𝕗𝖋𝖿𝗳𝘧𝙛𝚏',
    'g': 'ĝğġģǥǧǵɠɡɢɣγᵍᵞᵧᵷᶃḡℊ⒢ⓖｇ𝐠𝑔𝒈𝓰𝔤𝕘𝖌𝗀𝗴𝘨𝙜𝚐',
    'h': 'ĥħȟɦɧʮʯʰʱḣḥḧḩḫẖₕ⒣ⓗⱨｈ𝐡𝒉𝒽𝓱𝔥𝕙𝖍𝗁𝗵𝘩𝙝𝚑',
    'i': 'ìíîïĩīĭįıǐȉȋɨɩίιϊᴉᵎᵢḭḯỉịἰἱἲἳἴἵἶἷὶίιῐῑῒΐῖῗⁱⅈ⒤ⓘｉ𝐢𝑖𝒊𝒾𝓲𝔦𝕚𝖎𝗂𝗶𝘪𝙞𝚒𝚤',
    'j': 'ĵǰȷɉɟʄʝʲϳⅉ⒥ⓙｊ𝐣𝑗𝒋𝒿𝓳𝔧𝕛𝖏𝗃𝗷𝘫𝙟𝚓𝚥',
    'k': 'ķĸƙǩʞκϰᵏᶄḱḳḵₖ⒦ⓚⱪｋ𝐤𝑘𝒌𝓀𝓴𝔨𝕜𝖐𝗄𝗸𝘬𝙠𝚔',
    'l': 'ĺļľŀłƚƛȴɫɬɭˡλᶅḷḹḻḽₗℓ⒧ⓛⱡｌ𝐥𝑙𝒍𝓁𝓵𝔩𝕝𝖑𝗅𝗹𝘭𝙡𝚕',
    'm': 'ɱμᴟᵐᵚᵯᶆḿṁṃₘ⒨ⓜｍ𝐦𝑚𝒎𝓂𝓶𝔪𝕞𝖒𝗆𝗺𝘮𝙢𝚖',
    'n': 'ñńņňƞǹȵɲɳɴνᵰᶇṅṇṉṋⁿₙ⒩ⓝｎ𝐧𝑛𝒏𝓃𝓷𝔫𝕟𝖓𝗇𝗻𝘯𝙣𝚗',
    'o': 'òóôõöøōŏőơǒǫǭǿȍȏȫȭȯȱɔɵοωόώᵒṍṏṑṓọỏốồổỗộớờởỡợὀὁὂὃὄὅὠὡὢὣὤὥὦὧὸόὼώᾠᾡᾢᾣᾤᾥᾦᾧῲῳῴῶῷₒℴ⒪ⓞｏ𝐨𝑜𝒐𝓸𝔬𝕠𝖔𝗈𝗼𝘰𝙤𝚘',
    'p': 'ƥπϖᵖᵱᵽᶈṕṗₚ⒫ⓟｐ𝐩𝑝𝒑𝓅𝓹𝔭𝕡𝖕𝗉𝗽𝘱𝙥𝚙',
    'q': 'Ɋɋʠϟ⒬ⓠｑ𝐪𝑞𝒒𝓆𝓺𝔮𝕢𝖖𝗊𝗾𝘲𝙦𝚚',
    'r': 'ŕŗřȑȓɍɹɺɻɼɽɾɿʳʴʵʶρϱᵣᵨᵲᵳᶉṙṛṝṟ⒭ⓡｒ𝐫𝑟𝒓𝓇𝓻𝔯𝕣𝖗𝗋𝗿𝘳𝙧𝚛',
    's': 'śŝşšſșȿʂˢςσᵴᶊṡṣṥṧṩₛ⒮ⓢｓ𝐬𝑠𝒔𝓈𝓼𝔰𝕤𝖘𝗌𝘀𝘴𝙨𝚜',
    't': 'ţťŧƫƭțȶʇʈτᵗᵵṫṭṯṱẗₜ⒯ⓣⱦｔ𝐭𝑡𝒕𝓉𝓽𝔱𝕥𝖙𝗍𝘁𝘵𝙩𝚝',
    'u': 'ùúûüũūŭůűųưǔǖǘǚǜȕȗɤʉΰυϋύᴝᴞᵘᵙᵤṳṵṷṹṻụủứừửữựὐὑὒὓὔὕὖὗὺύῠῡῢΰῦῧ⒰ⓤｕ𝐮𝑢𝒖𝓊𝓾𝔲𝕦𝖚𝗎𝘂𝘶𝙪𝚞',
    'v': 'ʋᵛᵥᶌṽṿ⒱ⓥｖ𝐯𝑣𝒗𝓋𝓿𝔳𝕧𝖛𝗏𝘃𝘷𝙫𝚟',
    'w': 'ŵƿʍʷϝẁẃẅẇẉẘ⒲ⓦｗ𝐰𝑤𝒘𝓌𝔀𝔴𝕨𝖜𝗐𝘄𝘸𝙬𝚠',
    'x': 'ˣξᶍẋẍₓ⒳ⓧｘ𝐱𝑥𝒙𝓍𝔁𝔵𝕩𝖝𝗑𝘅𝘹𝙭𝚡',
    'y': 'ýÿŷƴȝȳɏʎʸẏẙỳỵỷỹ⒴ⓨｙ𝐲𝑦𝒚𝓎𝔂𝔶𝕪𝖞𝗒𝘆𝘺𝙮𝚢',
    'z': 'źżžƶȥɀʐʑζᵶᶎẑẓẕ⒵ⓩⱬｚ𝐳𝑧𝒛𝓏𝔃𝔷𝕫𝖟𝗓𝘇𝘻𝙯𝚣',
}
