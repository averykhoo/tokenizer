...
class BagOfWordsCorpus:
    ...
    def word_counts_(self, document_index: int, normalize: bool = False) -> Dict[str, int]:
        if normalize:
            total_words = self.num_words(document_index)
    ...
    def idf_(self, document_indices: Iterable[int], add_one_smoothing: bool = True) -> Dict[str, float]:
    ...
    def stopwords_(self, document_indices: Iterable[int], stopword_df=0.85) -> Set[str]:
        assert 0.0 < stopword_df <= 1.0
    ...
    def all_words_(self, document_index: int) -> Generator[str, Any, None]:
        for word_idx, count in zip(*self._corpus[document_index]):
            word = self.vocabulary[word_idx]
            for _ in range(count):
                yield word
    ...
    def unique_words_(self, document_index: int) -> List[str]:
    ...
    def num_words_(self, document_index: int) -> int:
